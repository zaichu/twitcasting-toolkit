...
- 無料コインが回復したらデスクトップ通知でお知らせ(popup からON/OFF切り替え可能)
...
ユーザーが選択したチェックボックス自動適用設定、無料コイン回復通知のON/OFF設定、通知判定に必要な直近のポイント状態を保存するために使用します。保存するのは拡張機能の動作に必要な設定・状態のみで、閲覧履歴、コメント、アイテム送信履歴は保存しません。

### alarms

無料コインの回復予定時刻、または一定間隔でのポイント状態確認をバックグラウンドでスケジュールするために使用します。タイマー実行以外の用途には使用しません。

### notifications

無料コインの回復が完了したタイミングを、OS のデスクトップ通知でユーザーに知らせるために使用します。ユーザーは popup からこの通知をいつでも無効化できます。
...
- 保存するデータは、チェックボックス自動適用のホスト別設定、無料コイン回復通知のON/OFF設定、通知判定用の直近ポイント状態のみです。
...
- `release/twitcasting-toolkit-2.0.5.zip` を Chrome Web Store にアップロード
...
Chrome Web Store に指定するプロモーションタイル:

- `docs/store-assets/promo-small.png` - 440x280 PNG
- `docs/store-assets/promo-marquee.png` - 1400x560 PNG
